...
<!--begin::App Main-->
      ...
      <!--end::App Main-->
<script>
function afficherSociete(id) {
    // Rediriger vers la page de détails de la société
    window.location.href = 'societe_details.php?id=' + id;
}

function modifierSociete(id) {
    // Rediriger vers la page de modification de la société
    window.location.href = 'modifier_societe.php?id=' + id;
}

function supprimerSociete(id) {
    if (confirm('هل أنت متأكد من حذف هذه الشركة؟')) {
        // Envoyer une requête AJAX pour supprimer la société
                        ...
                        <div class="tab-pane fade show active" id="info-generales" role="tabpanel">
                            <div class="row g-3">                                
                                ...
                                <div class="col-6">
                                    ...
                                    <input type="text" class="form-control" id="raison_fr" name="raison_fr" required placeholder=" مثال: DTP">
                                </div>
...
<!-- Style CSS additionnel pour le modal -->
<style>
    .modal-content {
        border-radius: 15px;
        border: none;
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const modalEl = document.getElementById('ajouterSocieteModal');
    const form = document.getElementById('formAjouterSociete');
    const resetBtn = document.getElementById('btnResetForm');
    const submitBtn = document.getElementById('btnSubmit');
    const messageAlert = document.getElementById('messageAlert');
    const defaultLogo = '../assets/images/default-company.png';
    ...
    resetBtn?.addEventListener('click', () => {
        ...
        const firstTab = document.querySelector('#societeTabs button');
        firstTab?.click();
    });
    ...
    form.addEventListener('submit', async (e) => {
        ...
        try {
            const response = await fetch('ajax/traitement_societe.php?action=ajouter', {
                method: 'POST',
    ...
    function validateForm() {
        ...
        if (email?.value && !/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email.value)) {
            ...
            showMessage('البريد الإلكتروني غير صالح', 'warning');
            return false;
        }
        ...
        if (!valid) {
            showMessage('يرجى ملء جميع الحقول الإلزامية', 'warning');
        }
