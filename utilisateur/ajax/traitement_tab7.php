...
try {
    ...
    switch ($action) {
        ...
        case 'update_tab7':
            ...
            if ($action == 'add_tab7') {
                $existing = Tableau7::existe_pour_societe_annee($id_societe, $annee);
                if ($existing && $statut == 'validé') throw new Exception('يوجد جدول مصادق عليه لهذه السنة');
                $tableau = new Tableau7();
                ...
                $tableau = Tableau7::trouve_par_id($id_tableau);
            ...
            foreach ($details_to_delete as $id_det) {
                $det = DetailTab7::trouve_par_id(intval($id_det));
                if ($det && $det->id_tab_7 == $id_tableau) $det->delete();
            }
            ...
            foreach ($details as $idx => $det_data) {
                ...
                $fields = [
                    'nbr_agent_formation_initiale', 'date_agent_formation_initiale', 'duree_agent_formation_initiale',
                    'nbr_agent_formation_supplementaire', 'date_agent_formation_supplementaire', 'duree_agent_formation_supplementaire',
                    'nbr_agent_formation_mise_niveau', 'date_agent_formation_mise_niveau', 'duree_agent_formation_mise_niveau',
                    'nbr_agent_formation_recyclage', 'date_agent_formation_recyclage', 'duree_agent_formation_recyclage',
                    'total', 'observations'
                ];
                $data_detail = [];
                foreach ($fields as $f) {
                    if ($f == 'date_agent_formation_initiale' || $f == 'date_agent_formation_supplementaire' ||
                        $f == 'date_agent_formation_mise_niveau' || $f == 'date_agent_formation_recyclage') {
                        $data_detail[$f] = isset($det_data[$f]) ? $det_data[$f] : null;
                    } else {
                ...
                if ($id_detail > 0) {
                    $obj = DetailTab7::trouve_par_id($id_detail);
                    if ($obj && $obj->id_tab_7 == $id_tableau) {
