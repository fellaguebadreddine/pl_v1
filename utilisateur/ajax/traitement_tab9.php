...
try {
    ...
    switch ($action) {
        ...
        case 'update_tab9':
            ...
            if ($action == 'add_tab9') {
                $existing = Tableau9::existe_pour_societe_annee($id_societe, $annee);
                if ($existing && $statut == 'validé') throw new Exception('يوجد جدول مصادق عليه لهذه السنة');
                $tableau = new Tableau9();
                ...
                $tableau = Tableau9::trouve_par_id($id_tableau);
            ...
            foreach ($details_to_delete as $id_det) {
                $det = DetailTab9::trouve_par_id(intval($id_det));
                if ($det && $det->id_tableau9 == $id_tableau) $det->delete();
            }
            ...
            foreach ($details as $idx => $det_data) {
                ...
                if ($id_detail > 0) {
                    $obj = DetailTab9::trouve_par_id($id_detail);
                    if ($obj && $obj->id_tableau9 == $id_tableau) {
