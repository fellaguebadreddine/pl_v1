...
// Tableau1.php
class DetailTab7 {
    protected static $nom_table="detail_tab_7";
	protected static $champs = array('id', 'id_tab_7', 'id_grade', 'id_societe', 'id_user',
     'annee', 'nbr_agent_formation_initiale', 'date_agent_formation_initiale', 'date_agent_formation_initiale',
      'duree_agent_formation_initiale', 'nbr_agent_formation_supplementaire', 'date_agent_formation_supplementaire',
       'duree_agent_formation_supplementaire' , 'nbr_agent_formation_mise_niveau', 'date_agent_formation_mise_niveau',
       'duree_agent_formation_mise_niveau' , 'nbr_agent_formation_recyclage', 'date_agent_formation_recyclage',
       'duree_agent_formation_recyclage', 'total', 'date_valide', 'observations');
    public $id;
    public $id_tab_7;
    public $id_grade;
    public $id_societe;
    public $id_user;
    public $annee;
    public $nbr_agent_formation_initiale;
    public $date_agent_formation_initiale;
    public $duree_agent_formation_initiale;
    public $nbr_agent_formation_supplementaire;
    public $date_agent_formation_supplementaire;
    public $duree_agent_formation_supplementaire;
    public $nbr_agent_formation_mise_niveau;
    public $date_agent_formation_mise_niveau;
    public $duree_agent_formation_mise_niveau;
     public $nbr_agent_formation_recyclage;
    public $date_agent_formation_recyclage;
    public $duree_agent_formation_recyclage;
    public $total;
    public $date_valide;
    public $observations;
   
public static function trouve_par_tableau($id_tab_7)
{
    global $bd;

    $sql = "SELECT *
            FROM " . self::$nom_table . "
            WHERE id_tab_7 = $id_tab_7
            ORDER BY id";

    $result = $bd->requete($sql, [$id_tab_7]);
    ...
    public static function trouve_par_grade_tableau($id_grade, $id_tab_7) {
        global $bd;
        $sql = "SELECT * FROM ".self::$nom_table." WHERE id_grade = id_grade AND id_tab_7 = $id_tab_7";
        $result = $bd->requete($sql, array($id_grade, $id_tab_7));
    ...
    public static function creer($data) {
        global $bd;
        $sql = "INSERT INTO ".self::$nom_table." (id_tab_7, annee, id_user, code, id_societe, id_grade, 
                postes_total, postes_reel, poste_intirim, poste_femme, difference, observations, date_tabl) 
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
        $result = $bd->requete($sql, array(
            $data['id_tab_7'], $data['annee'], $data['id_user'], $data['code'], $data['id_societe'], 
	...
	$q =  "SELECT * FROM ".self::$nom_table;
	$q .= " WHERE id_tab_7 = 0 and id_societe = {$id_societe}   and id_user = {$id}";
    return  self::trouve_par_sql($q);
	}
  ...
  public static function trouve_tab_vide_par_admin($id,$id_societe) {
		return self::trouve_par_sql("SELECT * FROM ".self::$nom_table ." where id_tab_7 = 0 and id_societe = {$id_societe}   and id_user = {$id} ");
  }
