...
public static function trouve_par_tableau($id_tableau_3)
{
    ...
    $sql = "SELECT *
            FROM " . self::$nom_table . "
            WHERE id_tableau_3 = $id_tableau_3
            ORDER BY id";

    $result = $bd->requete($sql, [$id_tableau_3]);
    ...
    public static function trouve_par_grade_tableau($id_grade, $id_tableau_3) {
        global $bd;
        $sql = "SELECT * FROM ".self::$nom_table." WHERE id_grade = id_grade AND id_tableau_3 = $id_tableau_3";
        $result = $bd->requete($sql, array($id_grade, $id_tableau_3));
    ...
    public static function creer($data) {
        global $bd;
        $sql = "INSERT INTO ".self::$nom_table." (id_tableau_3, annee, id_user, code, id_societe, id_grade, 
                postes_total, postes_reel, poste_intirim, poste_femme, difference, observations, date_tabl) 
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
        $result = $bd->requete($sql, array(
            $data['id_tableau_3'], $data['annee'], $data['id_user'], $data['code'], $data['id_societe'], 
  ...
  public static function trouve_tab_vide_par_admin($id,$id_societe) {
		return self::trouve_par_sql("SELECT * FROM ".self::$nom_table ." where id_tableau_3 = 0 and id_societe = {$id_societe}   and id_user = {$id} ");
  }
