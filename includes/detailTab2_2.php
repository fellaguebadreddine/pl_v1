...
// Tableau1.php
class DetailTab2_2 {
    protected static $nom_table="detail_tab_2_2";
	protected static $champs = array('id', 'id_societe', 'id_tab_2_2', 'annee', 'id_user', 'observations');
	public $id;	
	public $id_societe;
	public $id_tab_2_2;
	public $annee;
...
public static function trouve_par_tableau($id_tab_2_2)
{
    global $bd;

    $sql = "SELECT *
            FROM " . self::$nom_table . "
            WHERE id_tab_2_2 = $id_tab_2_2
            ORDER BY id";

    $result = $bd->requete($sql, [$id_tab_2_2]);
    ...
    public static function trouve_par_grade_tableau($id_grade, $id_tab_2_2) {
        global $bd;
        $sql = "SELECT * FROM ".self::$nom_table." WHERE id_grade = id_grade AND id_tab_2_2 = $id_tab_2_2";
        $result = $bd->requete($sql, array($id_grade, $id_tab_2_2));
    ...
    public static function creer($data) {
        global $bd;
        $sql = "INSERT INTO ".self::$nom_table." (id_tab_2_2, annee, id_user, code, id_societe, id_grade, 
                postes_total, postes_reel, poste_intirim, poste_femme, difference, observations, date_tabl) 
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
        $result = $bd->requete($sql, array(
            $data['id_tab_2_2'], $data['annee'], $data['id_user'], $data['code'], $data['id_societe'], 
...
public static function trouve_tableeu_vide($id_societe,$id){
	...
	$q .= " WHERE id_tab_2_2 = 0 and id_societe = {$id_societe}   and id_user = {$id}";
  ...
  public static function trouve_tab_vide_par_admin($id,$id_societe) {
		return self::trouve_par_sql("SELECT * FROM ".self::$nom_table ." where id_tab_2_2 = 0 and id_societe = {$id_societe}   and id_user = {$id} ");
  }
