...
// Tableau1.php
class Tableau6_1 {
    protected static $nom_table="tableau_6_1";
	protected static $champs = array('id', 'id_societe', 'statut', 'annee', 'date_valide', 'id_user',
     'date_creation', 'commentaire_admin','id_admin_validateur');
	public $id;	
	public $id_societe;
	public $statut;
	public $annee;
	...
	public static function existe_pour_societe_annee($id_societe, $annee) {
		...
		$sql = "SELECT id FROM tableau_6_1 
    ...
    public static function trouve_par_societe_annee($id_societe, $annee) {
        global $bd;
        $sql = "SELECT * FROM tableau_6_1 WHERE id_societe = $id_societe AND annee = '{$annee}'";
...
public static function trouve_par_criteres($criteres = []) {
    ...
    public static function get_annees_par_societe($id_societe) {
        global $bd;
        $sql = "SELECT DISTINCT annee FROM tableau_6_1 WHERE id_societe = $id_societe ORDER BY annee DESC";
        $result_array = self::trouve_par_sql($sql);
		return !empty($result_array) ? array_shift($result_array) : false;
    }
   ...
   return  self::trouve_par_sql($q);
	...
	public static function trouve_tableau_6_1_par_id($id=0) {
    $result_array = self::trouve_par_sql("SELECT * FROM ".self::$nom_table." WHERE id_societe={$id} LIMIT 1");
		return !empty($result_array) ? array_shift($result_array) : false;
  }
