...
// Tableau1.php
class DetailTab6 {
    protected static $nom_table="detail_tab_6";
	protected static $champs = array('id', 'id_tab_6', 'id_grade', 'id_societe', 'id_user',
     'annee', 'nom', 'prenom', 'date_naissance', 'date_retraite', 'date_valide', 'observations');
    public $id;
    public $id_tab_6;
    public $id_grade;
    public $id_societe;
    ...
    public $date_retraite;
    public $date_valide;
    public $observations;
   
public static function trouve_par_tableau($id_tab_6)
{
    global $bd;

    $sql = "SELECT *
            FROM " . self::$nom_table . "
            WHERE id_tab_6 = $id_tab_6
            ORDER BY id";

    $result = $bd->requete($sql, [$id_tab_6]);
    ...
    public static function trouve_par_grade_tableau($id_grade, $id_tab_6) {
        global $bd;
        $sql = "SELECT * FROM ".self::$nom_table." WHERE id_grade = id_grade AND id_tab_6 = $id_tab_6";
        $result = $bd->requete($sql, array($id_grade, $id_tab_6));
    ...
    public static function creer($data) {
        global $bd;
        $sql = "INSERT INTO ".self::$nom_table." (id_tab_6, annee, id_user, code, id_societe, id_grade, 
                postes_total, postes_reel, poste_intirim, poste_femme, difference, observations, date_tabl) 
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
        $result = $bd->requete($sql, array(
            $data['id_tab_6'], $data['annee'], $data['id_user'], $data['code'], $data['id_societe'], 
	...
	$q =  "SELECT * FROM ".self::$nom_table;
	$q .= " WHERE id_tab_6 = 0 and id_societe = {$id_societe}   and id_user = {$id}";
    return  self::trouve_par_sql($q);
	}
  ...
  public static function trouve_tab_vide_par_admin($id,$id_societe) {
		return self::trouve_par_sql("SELECT * FROM ".self::$nom_table ." where id_tab_6 = 0 and id_societe = {$id_societe}   and id_user = {$id} ");
  }
