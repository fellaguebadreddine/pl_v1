...
class DetailTab9 {
   ...
   public $id;
    ...
    public $cadre_juridique;
    public $temps_plein_1 = 0;
    public $temps_partiel_1 = 0;
    public $temps_plein_2 = 0;
...
public static function trouve_par_tableau($id_tableau9)
{
    ...
    $sql = "SELECT *
            FROM " . self::$nom_table . "
            WHERE id_tableau9 = $id_tableau9
            ORDER BY id";

    $result = $bd->requete($sql, [$id_tableau9]);
    ...
    public static function trouve_par_grade_tableau($id_grade, $id_tableau9) {
        global $bd;
        $sql = "SELECT * FROM ".self::$nom_table." WHERE id_grade = id_grade AND id_tableau9 = $id_tableau9";
        $result = $bd->requete($sql, array($id_grade, $id_tableau9));
    ...
    public static function creer($data) {
        global $bd;
        $sql = "INSERT INTO ".self::$nom_table." (id_tableau9, annee, id_user, code, id_societe, id_grade, 
                postes_total, postes_reel, poste_intirim, poste_femme, difference, observations, date_tabl) 
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
        $result = $bd->requete($sql, array(
            $data['id_tableau9'], $data['annee'], $data['id_user'], $data['code'], $data['id_societe'], 
	...
	$q =  "SELECT * FROM ".self::$nom_table;
	$q .= " WHERE id_tableau9 = 0 and id_societe = {$id_societe}   and id_user = {$id}";
    return  self::trouve_par_sql($q);
	}
  ...
  public static function trouve_tab_vide_par_admin($id,$id_societe) {
		return self::trouve_par_sql("SELECT * FROM ".self::$nom_table ." where id_tableau9 = 0 and id_societe = {$id_societe}   and id_user = {$id} ");
  }
