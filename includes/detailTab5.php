...
// Tableau1.php
class DetailTab5 {
    protected static $nom_table="detail_tab_5";
	protected static $champs = array('id', 'id_tableau_5','id_grade', 'id_societe', 'id_user', 'annee',
        'date_externe_concour_examen', 'date_externe_concour_diplome', 'date_externe_concour_recyclage',
        'date_interne_concours_profi', 'date_interne_examen_profi', 'date_interne_preparation_list',
        'date_concour_qualification', 'tabl_mise_niveau', 'comite_installation',
    ...
    public $id_user;
    public $annee;
    public $date_externe_concour_examen;
    public $date_externe_concour_diplome;
    public $date_externe_concour_recyclage;
    public $date_interne_concours_profi;
    public $date_interne_examen_profi;
    public $date_interne_preparation_list;
    public $date_concour_qualification;
    public $tabl_mise_niveau;
    public $comite_installation;
    public $date_concour_formation;
    public $autre;
    public $observations;
   
public static function trouve_par_tableau($id_tableau_5)
{
    global $bd;

    $sql = "SELECT *
            FROM " . self::$nom_table . "
            WHERE id_tableau_5 = $id_tableau_5
            ORDER BY id";

    $result = $bd->requete($sql, [$id_tableau_5]);
    ...
    public static function trouve_par_grade_tableau($id_grade, $id_tableau_5) {
        global $bd;
        $sql = "SELECT * FROM ".self::$nom_table." WHERE id_grade = id_grade AND id_tableau_5 = $id_tableau_5";
        $result = $bd->requete($sql, array($id_grade, $id_tableau_5));
    ...
    public static function creer($data) {
        global $bd;
        $sql = "INSERT INTO ".self::$nom_table." (id_tableau_5, annee, id_user, code, id_societe, id_grade, 
                postes_total, postes_reel, poste_intirim, poste_femme, difference, observations, date_tabl) 
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
        $result = $bd->requete($sql, array(
            $data['id_tableau_5'], $data['annee'], $data['id_user'], $data['code'], $data['id_societe'], 
	...
	$q =  "SELECT * FROM ".self::$nom_table;
	$q .= " WHERE id_tableau_5 = 0 and id_societe = {$id_societe}   and id_user = {$id}";
    return  self::trouve_par_sql($q);
	}
  ...
  public static function trouve_tab_vide_par_admin($id,$id_societe) {
		return self::trouve_par_sql("SELECT * FROM ".self::$nom_table ." where id_tableau_5 = 0 and id_societe = {$id_societe}   and id_user = {$id} ");
  }
