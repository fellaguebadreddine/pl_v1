...
// Tableau1.php
class DetailTab4 {
    protected static $nom_table="detail_tab_4";
	protected static $champs = array('id', 'id_tableau_4', 'id_grade', 'postes_vacants_externe', 'produit_formation_paramedicale', 'concours_sur_titre', 'debutants_contractuels', 'ouvriers_batiment_contractuels', 'methode_sur_titre', 'examen_mini', 'test_mini_ouvriers', 'postes_financiers_exploites', 'nombre_postes_financiers_exploites', 'observations');
    public $id;
    public $id_tableau_4;
    public $id_grade;
    public $postes_vacants_externe;
    public $produit_formation_paramedicale;
    public $concours_sur_titre;
    public $debutants_contractuels;
    public $ouvriers_batiment_contractuels;
    public $methode_sur_titre;
    public $examen_mini;
    public $test_mini_ouvriers;
    public $postes_financiers_exploites;
    public $nombre_postes_financiers_exploites;
    public $observations;
   
public static function trouve_par_tableau($id_tableau_4)
{
    global $bd;

    $sql = "SELECT *
            FROM " . self::$nom_table . "
            WHERE id_tableau_4 = $id_tableau_4
            ORDER BY id";

    $result = $bd->requete($sql, [$id_tableau_4]);
    ...
    public static function trouve_par_grade_tableau($id_grade, $id_tableau_4) {
        global $bd;
        $sql = "SELECT * FROM ".self::$nom_table." WHERE id_grade = id_grade AND id_tableau_4 = $id_tableau_4";
        $result = $bd->requete($sql, array($id_grade, $id_tableau_4));
    ...
    public static function creer($data) {
        global $bd;
        $sql = "INSERT INTO ".self::$nom_table." (id_tableau_4, annee, id_user, code, id_societe, id_grade, 
                postes_total, postes_reel, poste_intirim, poste_femme, difference, observations, date_tabl) 
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
        $result = $bd->requete($sql, array(
            $data['id_tableau_4'], $data['annee'], $data['id_user'], $data['code'], $data['id_societe'], 
	...
	$q =  "SELECT * FROM ".self::$nom_table;
	$q .= " WHERE id_tableau_4 = 0 and id_societe = {$id_societe}   and id_user = {$id}";
    return  self::trouve_par_sql($q);
	}
  ...
  public static function trouve_tab_vide_par_admin($id,$id_societe) {
		return self::trouve_par_sql("SELECT * FROM ".self::$nom_table ." where id_tableau_4 = 0 and id_societe = {$id_societe}   and id_user = {$id} ");
  }
