...
class DetailTab2 {
    ...
    'loi_consiel_employer','date_fin_consiel_employer', 'reference_consiel_employer_prolong',
     ...
     'reference_consiel_recours_prolong', 'date_fin_consiel_recours_prolong', 'observations');
	public $id;	
	public $id_societe;
	public $id_tab_2;
	public $annee;
	...
	public $date_fin_consiel_recours_prolong;
	public $observations;
   
public static function trouve_par_tableau($id_tab_2)
{
    global $bd;

    $sql = "SELECT *
            FROM " . self::$nom_table . "
            WHERE id_tab_2 = $id_tab_2
            ORDER BY id";

    $result = $bd->requete($sql, [$id_tab_2]);
    ...
    public static function trouve_par_grade_tableau($id_grade, $id_tab_2) {
        global $bd;
        $sql = "SELECT * FROM ".self::$nom_table." WHERE id_grade = id_grade AND id_tab_2 = $id_tab_2";
        $result = $bd->requete($sql, array($id_grade, $id_tab_2));
    ...
    public static function creer($data) {
        global $bd;
        $sql = "INSERT INTO ".self::$nom_table." (id_tab_2, annee, id_user, code, id_societe, id_grade, 
                postes_total, postes_reel, poste_intirim, poste_femme, difference, observations, date_tabl) 
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
        $result = $bd->requete($sql, array(
            $data['id_tab_2'], $data['annee'], $data['id_user'], $data['code'], $data['id_societe'], 
  ...
  public static function trouve_tab_vide_par_admin($id,$id_societe) {
		return self::trouve_par_sql("SELECT * FROM ".self::$nom_table ." where id_tab_2 = 0 and id_societe = {$id_societe}   and id_user = {$id} ");
  }
